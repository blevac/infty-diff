...
def get_config():
    ...
    run.name = 'infty_diff_pixel_no_enc'
    run.experiment = 'brain_mollified_256_pixel_no_enc'
    run.wandb_dir = ''
    run.wandb_mode = 'online'

    config.data = data = ConfigDict()
    data.name = 'brain'
    data.root_dir = "/csiNAS/brett/fastmri_brain_preprocessed_256/"
    data.img_size = FieldReference(256)
    data.channels = 2
    data.fid_samples = 5000
