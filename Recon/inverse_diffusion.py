...
import functools
import torch_dct
from inverse_utils import A_adjoint, A_forward, to_cplx, nrmse, fft, ifft
...
class DCTGaussianBlur(nn.Module):
    ...
    def inverse(self, x):
        x = torch_dct.apply_linear_2d(x, self.dct)
        x = x * 1 / (self.gaussian.to(x.dtype))
        x = torch_dct.apply_linear_2d(x, self.idct)
        return x

class InverseGaussianDiffusion(nn.Module):
    """
    ...
    def __init__(
        ...
        # ZF_recon = ifft(self.meas)
        # ZF_2ch = torch.view_as_real(ZF_recon[:,0,...]).permute(0,-1,-3,-2)
        # ZF_mol_2ch = self.mollifier(torch.tensor(ZF_2ch, dtype=torch.float32).cpu())
        # ZF_mol = ZF_mol_2ch[:,0,...] + 1j*ZF_mol_2ch[:,1,...]
        # self.y_mol = self.mask * fft(ZF_mol.cuda())
        # self.y_mol = self.y_mol
    ...
    def ddim_sample(
        ...
        #     with torch.no_grad():
        #         sample_cplx = to_cplx(sample)
        #         nrmse_loss = nrmse(self.gt_img, sample_cplx)
        #         print(f"time step: {t[0]}, NRMSE: {abs(nrmse_loss.item())}" )
        x_deblurred = self.mollifier.undo_wiener(x)
        # x_deblurred = self.mollifier.inverse(x)
        x_deblurred.requires_grad = True
        x_mol = self.mollifier(x_deblurred)
        # x.requires_grad = True

        # print('entered DDIM sample function')
        # print(x.dtype)
        # print(t.dtype)
        # print(model)
        out = self.p_mean_variance(
        ...
        # x_c_est_cplx = to_cplx(out["pred_xstart"])
        x_c_est_cplx = to_cplx(self.mollifier.undo_wiener(out["pred_xstart"]))
        # x_c_est_cplx = to_cplx(self.mollifier.inverse(out["pred_xstart"]))
        Ax = A_forward(image=x_c_est_cplx,maps=self.maps,mask=self.mask)
        res_error = torch.norm(Ax-self.meas)**2
        ...
        if torch.isnan(grad[0,0,0,0]):
            ...
            # sample = sample - (self.gamma/res_error.item()**.5) * self.mollifier(grad)
            # sample = sample - self.mollifier((self.gamma/res_error.item()**.5) * grad)
            sample = self.mollifier(self.mollifier.undo_wiener(sample) - (self.gamma/res_error.item()**.5) * grad)
            # sample = self.mollifier(self.mollifier.inverse(sample) - (self.gamma/res_error.item()**.5) * grad)
            # sample = sample - (self.gamma)/100 * grad
            # sample = sample - (self.gamma/res_error.item()**.5) * grad
            # sample = self.mollifier(sample) # re molify the sample

        #clean up gradients
        x_deblurred = x_deblurred.detach()
        # x = x.detach()
        if self.gt_img is not None:
            with torch.no_grad():
                sample_cplx = to_cplx(sample)
    ...
    def ddim_sample_loop(
        ...
        for sample in self.ddim_sample_loop_progressive(
            ...
            final = sample
            if return_all:
                all_samples.append(sample["sample"][0].float().cpu().detach())
                all_pred_xstarts.append(sample["pred_xstart"][0].float().cpu().detach())
        if return_all:
            return final["sample"], torch.stack(all_samples), torch.stack(all_pred_xstarts), final["pred_xstart"]
        else:
            return final["sample"], final["pred_xstart"]
    ...
    def ddim_sample_loop_progressive(
        ...
        if progress:
            ...
            indices = tqdm(indices)
        # print('noise init: ', img[0,0,0,0])
        # print('eta: ', eta)
        for i in indices:
            # if i == indices[-1]:
            #     continue
            t = torch.tensor([i] * shape[0], device=device)
            # with torch.no_grad(): # we need to start tracking gradients here for x_t
...
class InverseSpacedDiffusion(InverseGaussianDiffusion):
    """
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        base_diffusion = InverseGaussianDiffusion(**kwargs)  # pylint: disable=missing-kwoa
