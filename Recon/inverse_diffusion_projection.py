...
import functools
import torch_dct
from inverse_utils import A_adjoint, A_forward, to_cplx, to_real, nrmse, ifft, fft
...
class InverseGaussianDiffusion(nn.Module):
    ...
    def ddim_sample(
        ...
        # print(sample==mean_pred)

        # do projection on measurement via Song's paper
        x_c_est_cplx = to_cplx(self.mollifier.undo_wiener(sample))
        y_c = torch.sqrt(alpha_bar)*self.meas + torch.sqrt(1-alpha_bar) * torch.randn_like(self.meas)
        x_prime = self.projection(x_cur=x_c_est_cplx, y_cur=y_c, lambd=self.lambd, mask=self.mask, maps=self.maps)
        x_prime_2ch = to_real(x_prime)

        sample = sample + x_prime_2ch
    ...
    def projection(x_cur, y_cur, lambd, mask, maps):
        y_prime = lambd*y_cur+(1-lambd)*mask*fft(x_cur) + abs(mask -1)*fft(x_cur)
        x_prime = ifft(y_prime)
        return x_prime
